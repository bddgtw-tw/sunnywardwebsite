...
# CSS logic:
# 1. carousel-item masks the image, has aspect-ratio 4:3
# 2. img inside is scaled up to 1.35 and positioned slightly to the right to hide the left column.
#    Since the left column is roughly 30% wide, if we scale the image to 1.45, 
#    we can offset it to the left so the left 30% of the image (which has the pink header) is pushed off-screen.
#    To achieve this, we can set transform: scale(1.4) translate(-15%, 0px).
#    This naturally crops out the pink vertical bar on the left AND the title/footer.
css_to_inject = """
/* Slider Replaced with Navigation Arrows with CSS Masking */
.photo-carousel-wrap {
  margin-top: 1.5rem;
  width: 100%;
  position: relative;
...
.photo-carousel {
  display: flex;
  overflow-x: scroll; /* Keep scroll functional but hide scrollbars */
...
.carousel-item {
  flex: 0 0 100%; /* Show one image full width */
...
.carousel-item img {
  ...
  /* Apply non-destructive scaling and shifting to hide pink vertical bar, stars and headers */
  transform: scale(1.4) translate(-14%, 0px);
...
'''
    for idx, case in enumerate(lang_data):
        ...
        b_img = case['base_img']
        # We restore the image path to use the ORIGINAL collage images, but the CSS will dynamically crop out the borders!
        carousel_html = f'''
          <div class="photo-carousel-wrap">
            <button class="carousel-btn prev" onclick="scrollCarousel(this, -1)">&#10094;</button>
            <div class="photo-carousel">
              <div class="carousel-item"><img src="../_assets/projects/{b_img}.jpg" alt="Detail 1" loading="lazy"></div>
            </div>
...
print("Successfully updated to CSS masked dynamic cropping on raw images.")
